...
from verify_movement import test_robot_movement
...
class RectangularRoom(object):
    ...
    def clean_tile_at_position(self, pos, capacity):
        ...
        if (m,n) in self.tiles:
            self.tiles[(m,n)] = max(0, self.tiles[(m, n)] - capacity)
    ...
    def is_tile_cleaned(self, m, n):
        ...
        Note: The tile is considered clean only when the amount of dirt on this
              tile is 0.
        """
        return self.tiles.get((m, n), 0) == 0

    def get_num_cleaned_tiles(self):
        """
        Returns: an integer; the total number of clean tiles in the room
        """
        return sum(dirt == 0 for dirt in self.tiles.values())
...
class FurnishedRoom(RectangularRoom):
    ...
    def is_position_valid(self, pos):
        ...
        if not (0 <= tile_x < self.width and 0 <= tile_y < self.height):
            return False
        return not self.is_tile_furnished(tile_x, tile_y)
...
class StandardRobot(Robot):
    ...
    def update_position_and_clean(self):
        ...
        """
        # Get the new position based on the current direction and speed
        new_position = self.position.get_new_position(self.direction, self.speed)

        # Check if the new position is valid (inside the room and not on a furnished tile)
        if self.room.is_position_valid(new_position):
            # Move to the new position
            self.position = new_position
            # Clean the tile at the new position by the robot's cleaning capacity
            self.room.clean_tile_at_position(self.position, self.capacity)
        else:
            # If the new position is invalid, rotate to a random new direction
            self.direction = random.uniform(0, 360)


# Uncomment the lines below to see the implementation of StandardRobot in action
...
class FaultyRobot(Robot):
    ...
    def update_position_and_clean(self):
        ...
        """
        if self.gets_faulty():
            # Robot is faulty: change direction randomly and do not clean
            self.direction = random.uniform(0, 360)
        else:
            # Robot is not faulty: behave like StandardRobot
            new_position = self.position.get_new_position(self.direction, self.speed)
            if self.room.is_position_valid(new_position):
                # Valid move: update position and clean the tile
                self.position = new_position
                self.room.clean_tile_at_position(self.position, self.capacity)
            else:
                # Invalid position, change direction randomly
                self.direction = random.uniform(0, 360)

# Uncomment the line below to see the implementation of FaultyRobot in action
...
def run_simulation(num_robots, speed, capacity, width, height, dirt_amount, min_coverage, num_trials,
    ...
    for trial in range(num_trials):
        # Initialize the room and robots
        ...
        while True:
            # Move all robots and clean tiles
            for robot in robots:
                robot.update_position_and_clean()

            # Check coverage
            clean_coverage = room.get_num_cleaned_tiles() / room.get_num_tiles()
            if clean_coverage >= min_coverage:
                break

            time_steps += 1

        # Add the trial's time steps to the total
        total_time_steps += time_steps

    # Return the average number of time steps
